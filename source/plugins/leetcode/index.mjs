...
export default async function({login, q, imports, data, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    //Load inputs
    let {user, sections, "limit.skills": _limit_skills, "ignored.skills": _ignored_skills, "limit.recent": _limit_recent} =
      imports.metadata.plugins.leetcode.inputs({data, account, q})

    // 🔧 Forzar uso de tu usuario de LeetCode si no se detecta otro
    if (!user || user === ".user.login") {
      user = "JorgeNavarro"
      console.warn("⚠️  No se detectó usuario en configuración. Usando LeetCode: JorgeNavarro")
    }
    ...
    //Languages stats
    {
      ...
      const {data: {data: {matchedUser: {languageProblemCount: languages}}}} =
        await imports.axios.post("https://leetcode.com/graphql", {
          variables: {username: user},
          query: queries.leetcode.languages()
        })
    ...
    //Skills stats
    {
      ...
      const {data: {data: {matchedUser: {tagProblemCounts: skills}}}} =
        await imports.axios.post("https://leetcode.com/graphql", {
          variables: {username: user},
          query: queries.leetcode.skills()
        })
      ...
      result.skills = result.skills.filter(({name, category}) =>
        (imports.filters.text(name, _ignored_skills)) && (imports.filters.text(category, _ignored_skills)))
    ...
    //Problems
    {
      ...
      const {data: {data: {allQuestionsCount: all, matchedUser: {submitStatsGlobal: {acSubmissionNum: submissions}}}}} =
        await imports.axios.post("https://leetcode.com/graphql", {
          variables: {username: user},
          query: queries.leetcode.problems()
        })
    ...
    //Recent submissions
    {
      ...
      const {data: {data: {recentAcSubmissionList: submissions}}} =
        await imports.axios.post("https://leetcode.com/graphql", {
          variables: {username: user, limit: _limit_recent},
          query: queries.leetcode.recent()
        })
